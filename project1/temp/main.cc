...
int buildMatchingMachine(const vector<string> &words,
  ...
  for (uint32_t i = 0; i < words.size(); ++i) {
    ...
    for (uint32_t j = 0; j < keyword.size(); ++j) {
      ...
      // input a new state
      if (g.find(currentState) == g.end()) {
        g[currentState] = (int*)malloc(MAXC * sizeof(int));
        memset(g[currentState], -1, MAXC * sizeof(int) );
      }
  ...
  while (q.size()) {
    ...
    for (int c = 0; c <= highestChar - lowestChar; ++c) {
      ...
      if (g.find(state) == g.end()) {
        g[state] = (int*)malloc(MAXC * sizeof(int));
        memset(g[state], -1, MAXC * sizeof(int) );
      }
      ...
      if (g[state][c] != -1) {
        ...
        if (g.find(failure) == g.end()) {
          g[failure] = (int*)malloc(MAXC * sizeof(int));
          memset(g[failure], -1, MAXC * sizeof(int) );
        }
...
int main(void)
{
  ...
  keywords.push_back("pen");
  keywords.push_back("apple");
  keywords.push_back("pineapple");
  // keywords.push_back("his");

  string text("penpineappleapplepen");
