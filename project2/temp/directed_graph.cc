/** Metadata
 * Author               : Kim, Myeong Jae
 * File Name            : directed_graph.cc
 * Due date             : 2017-10-22
 * Compilation Standard : c++11 */

#include "directed_graph.h"

directed_graph::directed_graph (uint64_t number_of_nodes) {
  this->number_of_nodes = number_of_nodes;

  // node number is start from one, so I created one more set
  nodes = new std::unordered_set<uint64_t>[this->number_of_nodes + 1];
}

directed_graph::~directed_graph () {
  delete[] nodes;
}

bool directed_graph::is_node_exist(uint64_t node_number) {
  // check whether node_number is exist
  if(node_number > number_of_nodes) {
    std::cout << "(directed_graph::check_node_number) Failed. node_number is bigger than number_of_nodes" << std::endl;
    return false;
  }

  // node zero is not used.
  assert(node_number != 0);

  return true;
}

bool directed_graph::add_edge(uint64_t from, uint64_t to) {
  // check whether node is exist
  if ( ! (is_node_exist(from) && is_node_exist(to)) ) {
    return false;
  }

  // add an edge
  nodes[from].insert(to);
  return true;
}

bool directed_graph::remove_edge(uint64_t from, uint64_t to) {
  // check whether node is exist
  if ( ! (is_node_exist(from) && is_node_exist(to)) ) {
    return false;
  }

  // remove an edge
  nodes[from].erase(to);
  return true;
}

void directed_graph::show_all_edges() {
  uint64_t edge_count = 0;

  // node is start from one.
  std::cout << "** Printing All Edges **" << std::endl;

  // iterate all nodes
  for (uint64_t from = 1; from <= number_of_nodes; ++from) {

    // iterate all edges in each node
    for (auto to : nodes[from]) {
      std::cout << "Edge from "<< from << "\tto " << to << std::endl;
      edge_count++;
    }
  }

  std::cout << "The number of edges: " << edge_count << std::endl;
  std::cout << "**  Printing is end   **" << std::endl;
}


// This function only detects a cycle that has 'from' node.
// If there is another cycle that does not has 'from' node,
// it cannot detects it.
bool directed_graph::has_cycle_start_from(uint64_t from) {
  // check whether a node is exist
  if ( ! is_node_exist(from) ) {
    return false;
  }

  // BFS to find cycle
  std::queue<uint64_t> queue;
  std::unordered_set<uint64_t> *node_pointer;

  std::unordered_set<uint64_t> visited;

  // initialization
#ifdef GRAPH_DBG
  std::cout << "current node: " << from << std::endl;
#endif

  // Add nodes
  for (auto to : nodes[from]) {
    queue.push(to);
    visited.insert(to);
  }

  // BFS
  while ( !queue.empty() ) {
    uint64_t next_node = queue.front();
    queue.pop();

    // If next node is same with 'from',
    // it means that a cycle is exist!
    if (next_node == from) {
      // cycle found!
#ifdef GRAPH_DBG
      std::cout << "cycle found!" << std::endl;
#endif
      return true;
    }
    node_pointer = &nodes[next_node];

#ifdef GRAPH_DBG
    std::cout << "current node: " << next_node << std::endl;
#endif

    for (auto to : *node_pointer) {
      if (visited.find(to) != visited.end()) {
        // visited node. do not add.
        continue;
      }

      queue.push(to);
      visited.insert(to);
    }
  }


  // cycle not found
  return false;
}



// This function finds a cycle that includes 'from' node.
// If cycle is found, the return vector has member nodes of cycle.
// If not found, return vector is empty.
std::vector<uint64_t> directed_graph::get_cycle(uint64_t from) {
  // initialize data structures.
  
  // The stack is used to save cycle members.
  std::vector<uint64_t> stack;

  // The visited set has nodes that have been visited.
  std::unordered_set<uint64_t> visited;

  // DFS
  for (auto to : nodes[from]) {
    // push destination to stack
    stack.push_back(to);

#ifdef GRAPH_DBG
    std::cout << "(get_cycle) start finding cycle." << std::endl;
    std::cout << "(get_cycle) from " << from <<" to " << to << std::endl;
#endif

    // Call recursive function
    if (get_cycle_recur(from, stack, visited)) {

#ifdef GRAPH_DBG
      std::cout << "(get_cycle) cycle found" << std::endl;
      std::cout << "(get_cycle) " << stack[0];

      for (uint64_t i = 1; i < stack.size(); ++i) {
        std::cout << " -> " << stack[i];
      }
      std::cout << std::endl;

#endif

      return stack;
    }

    stack.clear();
  }

#ifdef GRAPH_DBG
    std::cout << "(get_cycle) cycle not found" << std::endl;
#endif


  return stack;
}


// This is a function used to find cycle in get_cycle.
// If cycle is found, it reutrns ture.
// If not, it returns false.
bool directed_graph::get_cycle_recur(uint64_t from,
                                std::vector<uint64_t> &stack,
                                std::unordered_set<uint64_t> &visited){
  // Base case
  uint64_t current_node = stack.back();

  visited.insert(current_node);

  if (nodes[current_node].empty()) {
    // There is no cycle.
    return false;
  } else if (current_node == from) {
    // cycle is found
    return true;
  }

  // Recursion process
  for (auto to : nodes[current_node]) {
    // do not visit node again
    if (visited.find(to) == visited.end()) {
      stack.push_back(to);

#ifdef GRAPH_DBG
      std::cout << "(get_cycle_recur) from " << current_node <<" to " << to << std::endl;
#endif

      if (get_cycle_recur(from, stack, visited)) {
        return true;
      }

      stack.pop_back();
    }
  }

  return false;
}
